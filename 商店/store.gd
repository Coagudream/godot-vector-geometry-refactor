...
func refresh_items() -> void:
	...
	for item in range(ITEMS_AMOUNT - item_lock):
		...
		new_item.already_is_buy.connect(updata_button_atate)
		items_container.add_child(new_item)
	updata_button_atate()
	item_lock = 0

##更新按钮状态
func updata_button_atate() -> void:
	can_or_not_can_refresh()
	can_or_not_can_buy()

##能不能按下刷新按钮
func can_or_not_can_refresh() -> void:
	if refresh_gem_amount > run_state.run_state_.gem_amount:
		refresh_buuton.disabled = true
	else :
		refresh_buuton.disabled = false

##判断能不能按下购买按钮
func can_or_not_can_buy() -> void:
	if not run_state:
		return
	for item in items_container.get_children():
		if item.items.gam_stone_spend > run_state.run_state_.gem_amount:
			...
			item.buy_botton.disabled = false
	can_or_not_can_refresh()
